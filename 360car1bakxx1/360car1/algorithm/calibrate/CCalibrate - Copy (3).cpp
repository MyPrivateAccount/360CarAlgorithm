...
bool CCalibrate::OnCalibrateX(Mat pMat , vector<vector<Point3f> > pRect , int&iCalibrateProcess )
{
	...
	//if ( !m_Points_World1.empty() )          // 如果此值不为空的话，那么就要将图像分割成两份分别进行角点检测
	//{
	//	int ikkk;
	//	ikkk = 10;
	//}
	if ( m_Points_World.size() == 2 )          // 如果有两个世界坐标的话，那么就将图像分割成两份分别进行角点检测
	{
		int ikkk;
		ikkk = 10;
	}
	...
	// cvtColor( pMat , imageGray , CV_RGB2GRAY );

	bReturn = findChessboardCorners( /*conimage_Mat*/ pMat , m_cornerDimension , corners , CALIB_CB_ADAPTIVE_THRESH|CALIB_CB_NORMALIZE_IMAGE|CALIB_CB_FAST_CHECK );
	...
	cornerSubPix( /*conimage_Mat*/pMat /*imageGray*/ , corners , Size( 11 , 11 ) , Size( -1 , -1 ) , TermCriteria( CV_TERMCRIT_EPS + CV_TERMCRIT_ITER , 30 , 0.1 ) );

	iCalibrateProcess++;

	// 判断坐标原点大约在图像上的哪个位置(左上？右上？左下？右下？)
	// 前判断是以列扫描方式还是行扫描方式
	...
	// 对m_Rect_Img进行亚像素操作
	RectImgSubPix( /*conimage_Mat*/pMat , m_Rect_Img );
...
void CCalibrate::RectImgSubPix( Mat grayImage , vector<vector<Point2f> >&rect_Point )
{
	...
	for ( int i = 6 ; i<12 ; i++ )
	{
		...
		{
			cornerSubPix( grayImage , tempPoint2[j] , Size(i,i) , Size(-1,-1) , TermCriteria(  CV_TERMCRIT_EPS + CV_TERMCRIT_ITER , 30 , 0.1 ) );
...
//			{
	...
	//////////////////////////////////////////////////////////////////////////
	// 自适应二值化
	cv::adaptiveThreshold( pMat , ThreshMat , 255 , CV_ADAPTIVE_THRESH_GAUSSIAN_C/*CV_ADAPTIVE_THRESH_MEAN_C*/ , CV_THRESH_BINARY , 201 , 0 );
	//////////////////////////////////////////////////////////////////////////
		...
		//		waitKey( 0 );
				...
				double high = 0.0;
				//adaptivefind
				AdaptiveFindThreshold( ThreshMedian/*ThreshMedian*//*grayMat*/ , &low , &high );         // 自动门限

				Canny( ThreshMedian/*ThreshMedian*//*grayMat*/ , canny_Mat , low , high );
	...
	//			waitKey( 0 );
				...
				if( !contourPoint.empty() && !hierarchy.empty() )
				{
					...
					{  
						...
						if ( area > maxArea )
						{
	//								drawContours(contoursImage,contourPoint,i,Scalar(0),3);  
	...
	//				waitKey( 0 );
				...
				for ( int yyy = 0 ; yyy<contourPointD.size() ; yyy++  )
				{
					...
					for ( int xxx = 0 ; xxx<roi_point_approx.size() ; xxx++ )
					{
						...
						cv::circle(roi_approx,xxx1,2,cv::Scalar(0,0,255));
					}
#endif
					i = i + 1;
...
void CCalibrate::OnDetermineRectInImg( vector<vector<Point> >&leftPoint , vector<vector<Point> >&rightPoint , vector<vector<Point2f> >&dst )
{
	...
	else
	{
		...
		{
			yDivision = yMin - 2*((yMax - yMin)/iTemp);
		}
...
//	imgPoint.push_back( position_Img );
	...
	vector<Point3f> positionTemppp = position_World.at( 0 );
	for ( int i = 0 ; i<worldPoint_Temp.size() ; i++ )
	{
		positionTemppp.push_back( worldPoint_Temp.at(i) );
	}
	
	vector<Point2f> imgPointsitionTemppp = position_Img;
...
//	imgPoint.push_back( imgPoint_Temp );
	...
	Vec2d _c( m_fishInf.innerParam.val[2] , m_fishInf.innerParam.val[6] );
	Vec2d _f( m_fishInf.innerParam.val[0] , m_fishInf.innerParam.val[5] );
	int flags = 0;
