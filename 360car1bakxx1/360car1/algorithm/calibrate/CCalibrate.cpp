...
{
    cv::Point3f pLocation;
	vector<cv::Point3f> pTempx;
...
// 进行重排
void CCalibrate::OnReSort_Square_Img(vector<vector<cv::Point2f> >&pTemp, int iSindex)
{
	vector<cv::Point2f> temp1 = pTemp.at(0);
	vector<cv::Point2f> temp2 = pTemp.at(1);
...
// 标定
bool CCalibrate::OnCalibrateX(cv::Mat pMat, vector<vector<cv::Point3f> > pRect, int&iCalibrateProcess)
{
	//imshow( "zhou" , pMat );
	//waitKey( 0 );
	// 先找pMat中的角点
	...
	cv::Mat conimage_Mat(pMat.rows, pMat.cols, CV_8U, cv::Scalar(0));
...
//	cv::adaptiveThreshold(pMat, ThreshMat, 255, CV_ADAPTIVE_THRESH_GAUSSIAN_C/*CV_ADAPTIVE_THRESH_MEAN_C*/, CV_THRESH_BINARY/*CV_THRESH_BINARY*/, 101, 0);
	...
	//cv::waitKey(0);
	//int n = pMat.channels();
	//vector<vector<cv::Point2f>> corners1;
	//cv::Size m_cornerDimension1(9, 6);

	m_Points_Img.clear();
	bReturn = cv::findChessboardCorners( /*conimage_Mat*/ pMat/*ThreshMat*/, m_cornerDimension, corners , cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_NORMALIZE_IMAGE | cv::CALIB_CB_FAST_CHECK);
	...
	if ( !bReturn )             // 如果角点没有找成功，那么就开始使用其他的方案
	{
		...
		for ( i = 130 ; i< 200 ; i= i+10 )
		{
			...
			threshold( /*conimage_Mat*/pMat, ThreshMat, i, 255, cv::THRESH_BINARY);

			bReturn = findChessboardCorners(ThreshMat, m_cornerDimension, corners, cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_NORMALIZE_IMAGE | cv::CALIB_CB_FAST_CHECK);
			if ( bReturn )
			{
	...
	if ( !bReturn )
	{
		...
		int j = 0;
		cv::Mat imageTemp = pMat.clone();

		for ( j = 0 ; j<corners.size() ; j++ )
		{
			circle(imageTemp, corners[j], 10, cv::Scalar(0, 0, 255), 2, 8, 0);//OpenCV中三彩色通道的顺序是BGR而不是RGB。
		}
	...
	cornerSubPix( /*conimage_Mat*/pMat, corners, cv::Size(11, 11), cv::Size(-1, -1), cv::TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 30, 0.1));
	...
	//	{
			...
			for ( int j = 0 ; j< m_Points_World.size() ; j++  )
			{
				...
				{
					cv::Point3f xf = pWorldPoint.at(xx);
			...
			{
				cv::Point2f gf = m_Points_Img.at(z);
	...
	for ( j = 0 ; j<m_Rect_Img.size() ; j++ )
	{
		...
		{
			circle(imageTemp, pTempppp.at(rrr), 2, cv::Scalar(0, 0, 255), 2);
		}
	}
...
// 亚像素化
void CCalibrate::RectImgSubPix(cv::Mat grayImage, vector<vector<cv::Point2f> >&rect_Point)
{
	//for ( int i = 0 ; i<m_Rect_Img.size() ; i++ )
	//{
	//	cornerSubPix( pMat /*imageGray*/ , m_Rect_Img.at(i) , Size( 7 , 7 ) , Size( -1 , -1 ) , TermCriteria( CV_TERMCRIT_EPS + CV_TERMCRIT_ITER , 30 , 0.1 ) );
	...
	// 6到12为窗口的大小
	vector<cv::Point2f> tempPoint1[2];
	vector<cv::Point2f> tempPoint2[2];
	vector<vector<cv::Point2f> > tempPoint3 = rect_Point;
	...
	for ( int i = 1 ; i < 12 ; i++ )
	{
		...
		{
			cornerSubPix(grayImage, tempPoint2[j], cv::Size(i, i), cv::Size(-1, -1), cv::TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 30, 0.01));
...
void CCalibrate::onFindMaxYAndMinY(vector<cv::Point2f> corners)
{
	float fTemp;
...
void CCalibrate::AdaptiveFindThreshold(const cv::Mat src, double *low, double *high, int aperture_size)
{                                                                                
...
bool CCalibrate::CheckRect(vector<cv::Point>&RectPoint, int imageWide, vector<vector<cv::Point> >&leftCorner, vector<vector<cv::Point> >&rightCorner)
{
	bool bReturn = true;
	...
	cv::Point         sTempMin;
	int           iTempMinIndex;
	vector<cv::Point> RectPoint_bak = RectPoint;
	vector<int>   vIndex;
...
bool CCalibrate::bCheckRectOnCorner(vector<cv::Point2f>  Points_Img, vector<cv::Point> PointLocation)
{
	int i = 0 ;
	int j = 0 ;
	bool bFinded = false;
...
// 求一定距离范围内的均值
void CCalibrate::iGetAverage(vector<vector<cv::Point2f> >&pPoint, int iMinIndex, vector<cv::Point2f>&average)
{
	vector<cv::Point2f> pMinPoint;
	vector<cv::Point2f> pCurrentPoint;      // 当前点
...
void CCalibrate::editPointRange(cv::Point&location, int rows, int cols)
{
	if (location.x < 0)
	{
		location.x = 0;
...
// 在二值化中查找边界点
bool CCalibrate::CheckBinaryPoint(cv::Point PointLocation, cv::Mat BinaryMat)
{
	// 在四邻域内查找
	bool bReturn = false;
	cv::Point PTemp;
...
bool CCalibrate::ergodicBinaryMat(cv::Point BeginPoint, cv::Mat BinaryMat, vector<cv::Point>&edgaPoint)
{
	bool bReturn = false;
	...
	cv::Point pPointBegin;
	cv::Point pPoint1, pPoint2;          // 遍历的点
	cv::Mat identifiIndex(BinaryMat.rows, BinaryMat.cols, CV_8UC1, cv::Scalar(0));           // 访问标识位，未被访问为0，以被访问为1
...
// 在大正方形轮廓图上查找4个顶点
void CCalibrate::GetRectPosFromContourPic(cv::Mat &erodeMat, vector<cv::Point> &Contour, bool bRight, vector<cv::Point> &roi_point_approx)
{
	bool bReturn = false;
	...
	vector<cv::Point> vpMinX;
	vector<cv::Point> vpMinY;
	vector<cv::Point> vpMaxX;
	vector<cv::Point> vpMaxY;

	int index[4] = { 0 };	// minX, maxX, minY, maxY

	vector<cv::Point> near;
	...
	//////////////////////////////////////////////////////////////////////////
	// 顶点精细化
	// 将正方形轮廓上查找到的横坐标和纵坐标的最大值和最小值相同的点压到vector
	vpMaxX.clear();
	vpMaxY.clear();
	vpMinX.clear();
	vpMinY.clear();
	...
	// 从各vector中筛选出需要的横坐标和纵坐标的最大值和最小值的点
	if (bRight)
	{
		pMaxY = vpMinX.front();
	...
	// 通过横坐标最大和最小的点连成直线，求出左边正方形左上角或者右边正方形右上角的点
	// 通过纵坐标最大的点，确定要求的点的区间
...
void CCalibrate::GetRectPosFromBinPic(cv::Mat &ThreshMat, cv::Mat &binMatBGR, cv::Mat &erodeMat, vector<cv::Point> &Contour, vector<cv::Point> &roi_point_approx)
{
	bool bReturn = false;
	...
	// 以图像正中间的那条竖线为准，提取出左边的正方形和右边的正方形
	...
	//////////////////////////////////////////////////////////////////////////
	// 在大正方形轮廓图上查找4个顶点
	vector<cv::Point> roi_point_contour;
	...
	//////////////////////////////////////////////////////////////////////////
	// 在二值化图像上查找邻域内的边缘点
	vector<vector<cv::Point>> vpVertexs;
	vector<cv::Point> vpTemp;
	...
	//////////////////////////////////////////////////////////////////////////
	// 顶点精细化
	// 将正方形边缘上查找到的横坐标和纵坐标的最大值和最小值相同的点压到vector
	vpMaxX.clear();
	vpMaxY.clear();
	vpMinX.clear();
	vpMinY.clear();
	...
	for (int j = 0; j < vpVertexs.at(2).size(); j++)
	{
		...
		{
			if ((vpVertexs.at(2).at(j).y == minY.y) || (vpVertexs.at(2).at(j).y == minY.y + 1))
				vpMinY.push_back(vpVertexs.at(2).at(j));
		}
	}
...
// 获取正方形在图像坐标系上的位置
bool CCalibrate::OnGetRectPosition(const cv::Mat pMat, const int iLocation, vector<vector<cv::Point2f> >&pRect)
{
	bool bReturn = false;
#if 1
	//Mat  grayMat;
	cv::Mat  dilate_Mat;                           // 存储膨胀后的图
	cv::Mat  erode_Mat;                            // 存储腐蚀后的图

	vector<vector<cv::Point> > leftCorner;          // 检测的所有的满足要求的正方形的点
	vector<vector<cv::Point> > rightCorner;         // 检测的所有的满足要求的正方形的点

	//cvtColor( pMat , grayMat , CV_RGB2GRAY );
	//vector<vector<vector<Point2f > > > pTempRect;
	vector<vector<cv::Point2f> > pRectLeft;
	vector<vector<cv::Point2f> > pRectRight;

	cv::Mat canny_Mat = pMat.clone();
...
//			{
	...
	//////////////////////////////////////////////////////////////////////////
	// 自适应二值化
	if (m_CalibrateScheme == TOURING_CAR)
	{
	...
	{
		cv::adaptiveThreshold(pMat, ThreshMat, 255, /*CV_ADAPTIVE_THRESH_GAUSSIAN_C*/CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY_INV/*CV_THRESH_BINARY*/, 321, 0);
	}
...
//	cv::adaptiveThreshold(pMat, ThreshMat, 255, CV_ADAPTIVE_THRESH_GAUSSIAN_C/*CV_ADAPTIVE_THRESH_MEAN_C*/, CV_THRESH_BINARY, 101, 0);
	...
	//////////////////////////////////////////////////////////////////////////
	// 形态学变换（膨胀）
	dilate(canny_Mat, dilate_Mat, element, cv::Point(-1, -1), 1);
	//////////////////////////////////////////////////////////////////////////
	// 形态学变换（腐蚀）
	erode(dilate_Mat, erode_Mat, element, cv::Point(-1, -1), 1);
	//////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////
	...
	cv::Mat erodeMat = erode_Mat.clone();
	...
	vector<vector<cv::Point> > contourPoint;
	vector<cv::Vec4i> hierarchy;
	vector<vector<cv::Point> > contourPointD;              // 存储大于5000面积的点
	double area, maxArea = 1000;      //设面积最大值大于5000Pixel 
	//double maxArea1 = 12000;
	cv::Mat contoursImage = pMat.clone();
	...
	if( !contourPoint.empty() && !hierarchy.empty() )
	{
		...
		{  
			...
			if ( area > maxArea )
			{
				drawContours(contoursImage, contourPoint, i, cv::Scalar(0), 1);
	...
	// 多边形拟合
#if 1
	vector<cv::Point> roi_point_approx;
	cv::Point xxx1;
	cv::Mat roi_approx(pMat.size(),CV_8UC3,cv::Scalar(0,0,0));

	cv::Mat hough_Mat = pMat.clone();
	cv::Mat dstImage;
	...
	for ( int yyy = 0 ; yyy<contourPointD.size() ; yyy++  )
	{
		...
		GetRectPosFromBinPic(ThreshMat, binMatBGR, erodeMat, *i, roi_point_approx);
...
bool CCalibrate::OnFindRectPointInImage(vector<vector<cv::Point> >&source, vector<vector<cv::Point2f> >&dst, int imageWide, int imageHeight, int iLocation)
{
	// Point的X代表宽，Y代表高
	//////////////////////////////////////////////////////////////////////////
	// 执行到这一步，棋盘区域的正方形肯定已经提取出来了，现在的目的就是怎样才能将棋盘区域的正方形准确的提取出来
	bool bReturn = true;
	vector<vector<cv::Point> > leftSource;          // 左图的一堆点，包含了正方形
	vector<vector<cv::Point> > leftCorner;          // 检测的所有的满足要求的正方形的点
	bool bLeft;

	vector<vector<cv::Point> > rightSource;         // 右图的一堆点，包含了正方形
	vector<vector<cv::Point> > rightCorner;         // 检测的所有的满足要求的正方形的点
	bool bRight;

	vector<cv::Point> sTemp;
	cv::Point         pTemp;
	int           iTemp = imageWide/2;                     // 中间的分界线
...
int CCalibrate::iGetMinDis2f(vector<vector<cv::Point2f> >&pPoint, float fThreath)
{
	float fTemp = 0;
...
int CCalibrate::iGetMinDis(vector<vector<cv::Point> >&pPoint, float fThreath)
{
	float fTemp = 0;

	vector<cv::Point> PointTemp;
...
void CCalibrate::OnDetermineRectInImg(vector<vector<cv::Point> >&leftPoint, vector<vector<cv::Point> >&rightPoint, vector<vector<cv::Point2f> >&dst)
{
	...
	vector<cv::Point> leftPointTemp;
	int iLeftIndex = 0;

	vector<cv::Point> rightPointTemp;
	int iRightIndex = 0;
	...
	cv::Point pTempRight;
	cv::Point2f fpTemp;
	vector<vector<cv::Point> > leftPoint1;
	vector<vector<cv::Point> > rightPoint1;
...
void CCalibrate::OnCheckRect(vector<vector<cv::Point> >&source, vector<vector<cv::Point> >&cornerLocation, bool bLeft)
{
	int i;
	int j;
	vector<cv::Point> pTemp;

	cv::Point pTopLeft;         // 左上
	int   iTopLeft_index = 0;

	cv::Point pTopRight;        // 右上
	int   iTopRight_index = 0;

	cv::Point pBottomRight;     // 右下
	int   iBottomRight_index = 0;

	cv::Point pBottomLeft;      // 左下
...
#else
void CCalibrate::OnCalibrate_private(const vector<vector<cv::Point3f> >&position_World, const vector<vector<cv::Point3f> >&Rect_World, const vector<cv::Point2f>&position_Img, const vector<vector<cv::Point2f> >&Rect_Img, CvSize imgSize, int&iProcess)
{
	vector<vector<cv::Point3f> > worldPoint;
	vector<vector<cv::Point2f> > imgPoint;

	vector<cv::Point3f> worldPoint_Temp;
	vector<cv::Point2f> imgPoint_Temp;
	...
	vector<cv::Point3f> positionTemppp = position_World.at(0);
	for (int i = 0; i<worldPoint_Temp.size(); i++)
	{
		positionTemppp.push_back(worldPoint_Temp.at(i));
	}

	vector<cv::Point2f> imgPointsitionTemppp = position_Img;
	for (int i = 0; i<imgPoint_Temp.size(); i++)
	{
		imgPointsitionTemppp.push_back(imgPoint_Temp.at(i));
	}

	// 标定内参和畸变系数
	//	worldPoint.push_back( worldPoint_Temp );
	worldPoint.push_back(positionTemppp);
	//	imgPoint.push_back( imgPoint_Temp );
	imgPoint.push_back(imgPointsitionTemppp);
	...
	cv::Vec2d _c(m_fishInf.innerParam.val[2], m_fishInf.innerParam.val[6]);
	cv::Vec2d _f(m_fishInf.innerParam.val[0], m_fishInf.innerParam.val[5]);
	int flags = 0;
	IntrinsicParams IntriParams;
	flags |= cv::fisheye::CALIB_RECOMPUTE_EXTRINSIC;
	...
	/*CCalibrateFisheye onBrate;*/
	std::vector<cv::Vec3d> omc, Tc, omc1, Tc1, TempOmc[2];
	cv::Mat rotation_matrix1 = cv::Mat(3, 3, CV_64FC1, cv::Scalar::all(0)); /* 保存每幅图像的旋转矩阵 */
	...
	for (i = 0; i<Rect_World.size(); i++)
	{
		...
		//	omc.push_back( omc1.at(1) );
		//	Tc.push_back( Tc1.at(1) );
		omc.push_back(omc1.at(0));
		Tc.push_back(Tc1.at(0));
...
bool CCalibrate::OnCalibrateInnerParam(vector<vector<cv::Point3f> >&worldPoint, vector<vector<cv::Point2f> >&imgPoint, cv::Matx34d&innerParam/*Matx33d&innerParam*/, cv::Matx14d&distor, cv::Size image_size)
{
	bool bReturn = true;
	...
	rms = cv::fisheye::calibrate(worldPoint, imgPoint, image_size, m_intrinsic_matrix, m_distortion_coeffs, m_rotation_vectors, m_translation_vectors, flags, cv::TermCriteria(2, 50, 1e-6));
...
void CCalibrate::OriginLocationAndErgodicMode(vector<cv::Point2f> cornerLocation, int CheckLen, OriginInImage&Location, ArrayModeInImage&ArrayMode)
{
	cv::Point2f pFist;
	cv::Point2f pLast;
	int iTempx;
...
void CCalibrate::OnReSort_Top(OriginInImage originLocation, ArrayModeInImage arrayMode, vector<cv::Point2f>&source, CvSize cornerSize, vector<cv::Point2f>&dst)
{
	int i;
	int j;
	int iTemp;
...
void CCalibrate::OnResort_Bottom(OriginInImage originLocation, ArrayModeInImage arrayMode, vector<cv::Point2f>&source, CvSize cornerSize, vector<cv::Point2f>&dst)
{
	int i;
	int j;
	int iTemp;
...
void CCalibrate::OnResort_Left(OriginInImage originLocation, ArrayModeInImage arrayMode, vector<cv::Point2f>&source, CvSize cornerSize, vector<cv::Point2f>&dst)
{
	int i;
	int j;
	int iTemp;
...
void CCalibrate::OnResort_Right(OriginInImage originLocation, ArrayModeInImage arrayMode, vector<cv::Point2f>&source, CvSize cornerSize, vector<cv::Point2f>&dst)
{
	int i;
	int j;
	int iTemp = 0;
...
// 获取当前鱼眼镜头内参
cv::Matx34d*CCalibrate::OnGetInnerParam()
{
	return &m_fishInf.innerParam;
}

// 获取当前鱼眼镜头的畸变系数
cv::Matx14d*CCalibrate::OnGetDistortionParam()
{
	return &m_fishInf.distortionParam;
}

// 获取外参,iCount表示获取的外参的个数
cv::Matx44d CCalibrate::OnGetRTParam(int iCount)
{
//	iCount = 2;      // 两个外参
...
vector<vector<cv::Point2f> > CCalibrate::OnGetRect_Img()
{
	return m_Rect_Img;
}
CCalibrate::~CCalibrate()
