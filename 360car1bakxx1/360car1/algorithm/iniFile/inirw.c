...
//#define _CRT_SECURE_NO_WARNINGS
	...
	int kk = sizeof(gFilename);
	//strcpy_s(gFilename, strlen(filename), filename);

	//file = fopen(gFilename, "rb");
	fopen_s(&file, gFilename, "rb");
	if (file == NULL) 
		return 0;
...
//					strncpy_s(value, len, value0, 100);
					value[len] = 0;
				}
				return 1;
			}
...
//			strncpy_s( value, len , defvalue , 100 );
			value[len] = 0;
		} else {
			value[0] = 0;
		}
...
int iniSetInt(const char *section, const char *key, int value, int base)
{
	...
	switch (base) {
	case 16:
		sprintf_s(valstr, "0x%x", value);
		return iniSetString(section, key, valstr);
	case 8:
		sprintf_s(valstr, "0%o", value);
		return iniSetString(section, key, valstr);
	default:	//10
